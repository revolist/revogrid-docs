...
const isArchiveBuild =
  truthyEnv(process.env.DOCS_ARCHIVE) || archiveHostnames.has(siteHostname(siteUrl) || '')
...
const config: UserConfig<DefaultTheme.Config> = {
  ...
  vue: {
    template: {
      compilerOptions: {
        isCustomElement: tag => tag === 'financial-pivot-header',
      },
    },
  },
  title: 'RevoGrid',
...
'https://www.googletagmanager.com/gtm.js?id='+i+dl;f.parentNode.insertBefore(j,f);
  ...
  transformPageData(pageData) {
    ...
    const authoredFaq = frontmatter.faq as
      { heading?: string; items?: Array<{ q?: string; a?: string }> } | undefined
    const nestedFaq = (
      frontmatter.ganttLanding as
        { faq?: { items?: Array<{ q?: string; a?: string }> } } | undefined
    )?.faq
  ...
  transformHead({ pageData, siteData }) {
    ...
    const description =
      ...
      headMetaContent(head, 'name', 'description') ||
  ...
  themeConfig: {
    ...
    editLink: {
      pattern: 'https://github.com/revolist/revogrid-docs/edit/main/:path',
    ...
    plugins: [
      {
        ...
        resolveId(source, importer) {
          ...
          return sourcePath ? `${sourcePath}${revogridDemoDataSuffix}` : null
        },
        async load(id) {
...
<!-- End Google Tag Manager (noscript) -->`,
    ...
    resolve: {
      ...
      alias: [
        ...
        {
          find: /^.*\/VPImage\.vue$/,
          replacement: fileURLToPath(new URL('./theme/VPImage.vue', import.meta.url)),
        },
        {
          find: /^.*\/VPFooter\.vue$/,
          replacement: fileURLToPath(new URL('./theme/VPFooter.vue', import.meta.url)),
        },
        {
          find: '@/',
