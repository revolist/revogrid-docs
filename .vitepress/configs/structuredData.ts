...
const breadcrumbJsonLd = (siteUrl: string, relativePath: string, title: string): JsonLdData => {
...
const offerJsonLd = (
  siteUrl: string,
  planId: 'open-source' | 'pro-lite' | 'pro-advanced',
): JsonLdData => {
...
const softwareApplicationJsonLd = (
  ...
  const pagePath =
    relativePath === 'index.md' && productId !== 'revogrid'
      ? product.pageUrl
  ...
  return {
    ...
    offers: getProductOfferPlanIds(productId)
      .filter(
        (planId): planId is 'open-source' | 'pro-lite' | 'pro-advanced' => planId !== 'enterprise',
      )
...
export const createStructuredDataHead = ({
  ...
  const head = [jsonLdScript('breadcrumb-json-ld', breadcrumbJsonLd(siteUrl, relativePath, title))]
  ...
  if (productId) {
    head.push(
      jsonLdScript(
        ...
        softwareApplicationJsonLd(siteUrl, relativePath, productId),
      ),
    )
  }
