...
Internal scenario catalog for **21 canonical demos**, with one document per catalog ID. Read each demo together with [shared-shell.md](shared-shell.md). The primary workflows are implemented in the docs E2E suite. [Observed issues](observed-issues.md) records resolved findings and their verification evidence.
...
The [Core/Pro docs E2E suite](../core-pro-scenarios.spec.ts) passed 28 tests on 2026-09-10, covering the executable Core/Pro flows recorded in the individual documents. Existing [docs E2E](../demo-experience.spec.ts), [configuration](../../../../playwright.docs.config.ts), and [source mapping unit tests](../../../unit/vitepress/theme/demoSources.test.ts) provide complementary shell and registry coverage. Remaining source-derived cases stay explicitly marked until separately executed.
