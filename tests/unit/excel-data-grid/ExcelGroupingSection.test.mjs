...
const groupingSection = readFileSync(
  new URL('../../../excel-data-grid/ExcelGroupingSection.vue', import.meta.url),
  'utf8',
)
const styles = readFileSync(
  new URL('../../../excel-data-grid/excelLanding.scss', import.meta.url),
  'utf8',
)
...
test('indents grouping leaf rows by the active grouping depth', () => {
  ...
  assert.match(
    groupingSection,
    /class: 'excel-group-leaf-value',[\s\S]*?'--excel-group-leaf-indent': leafIndent/,
  )
  assert.match(groupingSection, /prop: 'region',[\s\S]*?cellTemplate: groupLeafCellTemplate/)
  assert.match(
    styles,
    /\.excel-group-leaf-value \{[^}]*display: block;[^}]*margin-inline-start: var\(--excel-group-leaf-indent, 0\);/,
  )
})
...
test('keeps fixed grouping without the editable group panel', () => {
  assert.doesNotMatch(
    groupingSection,
    /ColumnGroupPanelPlugin|column-group-panel|grouping-change|onGroupingChange/,
  )
  ...
  assert.doesNotMatch(
    styles,
    /\.excel-live-grid--grouping \.group-panel|\.excel-live-grid--grouping \.group-item/,
  )
