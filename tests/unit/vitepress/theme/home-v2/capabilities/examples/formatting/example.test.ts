...
import { formattingExample } from '../../../../../../../../.vitepress/theme/home-v2/capabilities/examples/formatting/example'
import {
  budgetValueFormat,
  projectFormattingConfig,
} from '../../../../../../../../.vitepress/theme/home-v2/capabilities/examples/formatting/formatting'
...
describe('Data formatting example', () => {
  it('uses the RevoGrid Pro 2.7.15 coordinate formatting contract', () => {
    ...
    expect(projectFormattingConfig.cells?.[0]).toEqual(
      expect.objectContaining({
        range: { start: { row: 0, column: 0 } },
      }),
    )
    expect(budgetValueFormat).toEqual(
      expect.objectContaining({
        kind: 'preset',
        preset: 'currency',
      }),
