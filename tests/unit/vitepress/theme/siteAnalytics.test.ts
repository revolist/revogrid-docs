...
test('writes safe Google Tag Manager dataLayer events only once per lifecycle key', () => {
  ...
  assert.equal(
    trackSiteAnalytics('demo_view', { demo_id: 'filtering' }, 'demo_view:filtering', windowTarget),
    true,
  )
  assert.equal(
    trackSiteAnalytics('demo_view', { demo_id: 'filtering' }, 'demo_view:filtering', windowTarget),
...
test('accepts only a constrained experiment variant', () => {
  assert.equal(
    getAnalyticsExperimentVariant(target('?experiment_variant=pricing_a').location),
    'pricing_a',
  )
  assert.equal(
    getAnalyticsExperimentVariant(target('?experiment_variant=<email@example.com>').location),
    undefined,
  )
})
...
test('exposes the same safe payload in explicit analytics debug mode', () => {
  ...
  assert.deepEqual(calls, [
    ['[analytics]', JSON.stringify({ placement: 'hero_install', event: 'trial_install_click' })],
  ])
})
