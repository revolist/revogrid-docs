...
const highlightsSource = readFileSync(
  new URL('../../../pro/landing/ProFeatureHighlights.vue', import.meta.url),
  'utf8',
)
const heroSource = readFileSync(
  new URL('../../../pro/landing/ProHeroSection.vue', import.meta.url),
  'utf8',
)
...
test('presents Kanban as a Pro Advanced tool teams do not have to build', () => {
  ...
  assert.ok(
    existsSync(
      new URL('../../../public/blog/kanban-product-development-polished.png', import.meta.url),
    ),
  )
})
...
test('introduces Kanban workflows in the canonical Pro page metadata and hero', () => {
  ...
  assert.match(pageSource, /^ {4}subtitle: .*Kanban workflows/m)
})
...
test('includes Kanban and Scheduler in the 90-second Pro demo playlist', () => {
  ...
  for (const [src, label] of expectedDemos) {
    assert.match(
      heroSource,
      new RegExp(`src: '${src.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}', label: '${label}'`),
    )
