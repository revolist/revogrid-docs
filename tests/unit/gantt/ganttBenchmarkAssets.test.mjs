...
const componentSource = readFileSync(
  new URL('../../../.vitepress/theme/GanttBenchmarkResults.vue', import.meta.url),
  'utf8',
)
...
test('the Gantt benchmark report owns all result and media assets', () => {
  ...
  for (const name of [
    'latest.json',
    'latest.csv',
    'reference-2026-08-10.json',
    'reference-2026-08-10.csv',
    'gantt-benchmark-reference.png',
    'gantt-benchmark-walkthrough.webm',
  ]) {
  ...
  assert.deepEqual(
    readFileSync(new URL('gantt-benchmark-reference.png', publicDirectory)).subarray(0, 8),
    Buffer.from([0x89, 0x50, 0x4e, 0x47, 0x0d, 0x0a, 0x1a, 0x0a]),
  )
  assert.equal(
    readFileSync(new URL('gantt-benchmark-walkthrough.webm', publicDirectory))
      .subarray(0, 4)
      .toString('hex'),
    '1a45dfa3',
  )
