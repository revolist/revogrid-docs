...
function applyState(columns: ColumnRegular[], state: SavedGridState) {
  const order = new Map((state.columnOrder ?? []).map((prop, index) => [prop, index]))
  const sorting = new Map(
    (state.sorting?.columns ?? []).map(column => [String(column.prop), column.order]),
  )
...
function getColumnWidths(columns: ColumnRegular[]): Record<string, number> {
  return Object.fromEntries(
    columns.flatMap(column =>
      typeof column.size === 'number' ? [[String(column.prop), column.size]] : [],
    ),
  )
}
...
function updateStatus(state = loadState()) {
  ...
  status.textContent = savedKeys.length ? `Saved: ${savedKeys.join(', ')}` : 'No saved layout'
}
...
function render() {
  ...
  document.getElementById('state-persistence-save')?.addEventListener('click', async () => {
  ...
  document.getElementById('state-persistence-reset')?.addEventListener('click', () => {
