...
| `focusTemplate`               | `focus-template`  | Focus template custom function. Can be used to render custom focus layer. | `((createElement: HyperFunc<VNode>, detail: FocusRenderEvent) => any) \| null` | `null`      |
| `rowType` _(required)_        | `row-type`        | Row type                                                                  | `"rgRow" \| "rowPinEnd" \| "rowPinStart"`                                      | `undefined` |
| `selectionStore` _(required)_ | `selection-store` | Selection, range, focus for selection                                     | `ObservableMap<SelectionStoreState>`                                           | `undefined` |
