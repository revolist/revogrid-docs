...
const result = await page.evaluate(
  ...
  `
    ...
    const initialRender = new Promise(resolve => {
      grid.addEventListener('aftergridrender', () => resolve(performance.now() - initialStart), {
        once: true,
      })
    })
...
const output = {
  ...
  rendererComplexity:
    'Mixed renderer harness: 1 pinned custom name cell with status badge, 1 custom status badge column, 8 custom formatted numeric columns, 90 plain numeric columns.',
  machine,
  metrics: {
