    ...
    <div class="excel-container">
      ...
      <div class="excel-filter-demo">
        <div class="excel-filter-toolbar" aria-label="Interactive filters">
          ...
          >
            Region: {{ region }} <small>{{ activeRegions.includes(region) ? '✓' : '+' }}</small>
          </button>
          <button
          ...
          >
            Status: {{ status }} <small>{{ activeStatuses.includes(status) ? '✓' : '+' }}</small>
          </button>
          <button class="excel-sort-button" type="button" @click="toggleSort">
            ARR <small>{{ sortDirection === 'desc' ? '↓ desc' : '↑ asc' }}</small>
          </button>
          <span class="excel-filter-count"
            >{{ visibleCustomers.length }} of {{ customers.length }} rows</span
          >
        </div>
        <ExcelRevoGrid
          ref="filterGrid"
        ...
        <div class="excel-table-status">
          <div>
            <span>{{ visibleCustomers.length }} of {{ customers.length }} rows</span
            ><span>Total ARR {{ formatMoney(filteredArr) }}</span>
          </div>
...
import { AdvanceFilterPlugin, FIlTER_SELECTION, FilterHeaderPlugin } from '@revolist/revogrid-pro'
...
const customerColumns = [
  ...
  { prop: 'segment', name: 'Segment', size: 130, filter: [FIlTER_SELECTION] },
  {
    ...
    dataGridFormat: createBadgeFormat(statusStyles),
  },
  {
    ...emptyFilterHeader,
    prop: 'arr',
...
const customers = [
  ...
  {
    customer: 'Halberd Manufacturing',
    region: 'EMEA',
    segment: 'Mid-market',
    status: 'At risk',
    arr: 96500,
  ...
  {
    customer: 'Meridian Energy Partners',
    region: 'EMEA',
    segment: 'Enterprise',
    status: 'Active',
    arr: 312750,
  ...
  {
    customer: 'Brightwater Utilities',
    region: 'EMEA',
    segment: 'Enterprise',
    status: 'Active',
    arr: 268400,
  ...
  {
    customer: 'Corvus Retail Systems',
    region: 'APAC',
    segment: 'Mid-market',
    status: 'Churned',
    arr: 128400,
...
const filterGrid = ref<{
  applyFilterItems: (items: Record<string, unknown[]>) => Promise<void>
} | null>(null)
const filterConfig = {
  selection: { sortDirection: 'asc', sourceRowTypes: ['rgRow'] },
}
const sortedCustomers = computed(() =>
  filterAndSortCustomers(customers, 'All', customerStatuses, sortDirection.value),
)
const visibleCustomers = ref(
  filterAndSortCustomers(customers, 'EMEA', activeStatuses.value, sortDirection.value),
)
const filteredArr = computed(() =>
  visibleCustomers.value.reduce((total: number, row: { arr: number }) => total + row.arr, 0),
)
...
function onGridFilterChange(payload: {
  ...
  const next = readCustomerAdvancedFilterState(payload.filterItems, {
    regions,
    statuses: customerStatuses,
  })
