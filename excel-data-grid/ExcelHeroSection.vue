    ...
    <div class="excel-container excel-hero__inner">
      <div class="excel-hero__copy">
        ...
        <div class="excel-actions">
          <ProDocButton :href="content.primary.href" arrow>{{
            content.primary.label
          }}</ProDocButton>
          <a class="excel-text-link" :href="content.secondary.href">{{
            content.secondary.label
          }}</a>
        </div>
        <div class="excel-hero__proof" aria-label="Product facts">
          <span v-for="fact in content.facts" :key="fact.label"
            ><strong>{{ fact.value }}</strong> {{ fact.label }}</span
          >
        </div>
      </div>
      ...
      <div class="excel-workbench-frame">
        <div class="excel-frame-bar">
          ...
          <div class="excel-frame-bar__chips" aria-label="Grid state">
            <span class="excel-frame-chip excel-frame-chip--accent"
              >Region = {{ heroSelection.region }}</span
            >
            <span class="excel-frame-chip"
              >{{ heroSelection.count }}
              {{ heroSelection.count === 1 ? 'cell' : 'cells' }} selected</span
            >
          </div>
        </div>
        ...
        <div class="excel-frame-status" aria-label="Grid status">
          <div>
            <span>{{ heroSummary.count.toLocaleString('en-US') }} rows</span
            ><span>Sum {{ heroSummary.sum.toLocaleString('en-US') }}</span
            ><span>Avg {{ heroSummary.average.toLocaleString('en-US') }}</span>
          </div>
...
const heroColumns = [
  ...
  { name: 'Owner', prop: 'owner', size: 150, sortable: true },
  {
    ...
    size: 140,
    sortable: true,
    dataGridFormat: createBadgeFormat(heroStageStyles),
  },
  { name: 'ARR', prop: 'arr', size: 130, sortable: true, dataGridFormat: excelNumberFormat },
  {
    ...
    size: 120,
    sortable: true,
    dataGridFormat: excelPercentFormat,
  },
  {
    name: 'Close date',
    prop: 'closeDate',
    size: 150,
    sortable: true,
    dataGridFormat: excelMonoFormat,
  },
  {
    name: 'Region',
    prop: 'region',
    size: 258,
    sortable: true,
    columnProperties: () => ({ class: { 'excel-hero-region-header': true } }),
  },
]
...
const heroSeedRows = [
  ...
  {
    account: 'Meridian Energy Partners',
    owner: 'R. Okafor',
    stage: 'Proposal',
    arr: 312750,
    probability: 0.65,
  ...
  {
    account: 'Kestrel Media Networks',
    owner: 'S. Lindqvist',
    stage: 'Negotiation',
    arr: 158900,
    probability: 0.5,
...
function onHeroSelectionChange(payload: {
