...
test('brands the JavaScript Kanban board consistently as RevoGrid Kanban', () => {
  assert.ok(brandedSources.some((source) => /RevoGrid Kanban/i.test(source)))
  assert.match(pageSource, /^title: "JavaScript Kanban Board Component \| RevoGrid Kanban"$/m)
  assert.match(pageSource, /^titleTemplate: false$/m)
  assert.match(pageSource, /^description: RevoGrid Kanban is a virtualized JavaScript Kanban board component/m)
  assert.match(pageSource, /href: https:\/\/rv-grid\.com\/kanban/)
  assert.match(pageSource, /content: RevoGrid Kanban – JavaScript Kanban Board Component/)
  assert.match(pageSource, /eyebrow: RevoGrid Kanban/)
  assert.match(pageSource, /title: 'RevoGrid Kanban: JavaScript Kanban for your product\.'/)
})
