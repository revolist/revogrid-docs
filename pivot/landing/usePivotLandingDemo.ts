...
export function usePivotLandingDemo(agg: Ref<AggMode>) {
  const values = (filter: Partial<PivotRawRow>) =>
    RAW.filter(row =>
      Object.entries(filter).every(([key, value]) => row[key as keyof PivotRawRow] === value),
    ).map(row => row.rev)
  ...
  const productValue = (region: string, product: string, quarter: string) =>
    aggregate(values({ region, product, quarter }))
  const regionGrandTotal = (region: string) => aggregate(values({ region }))
  const productGrandTotal = (region: string, product: string) =>
    aggregate(values({ region, product }))
  const quarterGrandTotal = (quarter: string) => aggregate(values({ quarter }))
  const grandTotal = computed(() => aggregate(RAW.map(row => row.rev)))
  const maxRegionQuarter = computed(() =>
    Math.max(...REGIONS.flatMap(region => QUARTERS.map(quarter => regionTotal(region, quarter)))),
  )
  ...
  const barWidth = (value: number) =>
    `${Math.max(3, Math.round((value / maxRegionQuarter.value) * 56))}px`
