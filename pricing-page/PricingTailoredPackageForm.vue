  ...
  >
    <div class="tailored-form">
      <div v-if="submissionState === 'succeeded'" class="success-state" role="status">
        ...
        <button ref="successDoneButton" class="rg-btn" type="button" @click="emit('close')">
          Done
        </button>
      </div>
      ...
      <template v-else-if="step === 1">
        ...
        <form @submit.prevent="continueToDetails">
          ...
          <div class="form-actions">
            ...
            >
              Continue<span v-if="selectedCapabilities.length">
                ({{ selectedCapabilities.length }})</span
              >
            </button>
          </div>
        </form>
      ...
      <template v-else>
        ...
        <ul
          v-if="selectedCapabilityDetails.length"
...
const capabilities: Capability[] = [
  {
    ...
    description: 'Custom editors, formulas and rich cell input',
    icon: 'grid',
  },
  {
    id: 'filtering-search',
    name: 'Filtering & search',
    description: 'Advanced filters, multi-filter and search',
    icon: 'filter',
  },
  {
    id: 'grouping-tree-data',
    name: 'Grouping & tree data',
    description: 'Hierarchies, grouped rows and master-detail',
    icon: 'box',
  },
  {
    id: 'validation-workflows',
    name: 'Validation & workflows',
    description: 'Rules, statuses, events and approvals',
    icon: 'listCheck',
  },
  {
    id: 'excel-like-interactions',
    name: 'Excel-like interactions',
    description: 'Selection, fill, copy/paste and keyboard use',
    icon: 'grid',
  },
  {
    id: 'export-reporting',
    name: 'Export & reporting',
    description: 'CSV export and report-ready data views',
    icon: 'chart',
  },
  {
    id: 'remote-data-performance',
    name: 'Remote data & performance',
    description: 'Large datasets, server workflows and caching',
    icon: 'pulse',
  },
  {
    id: 'undo-redo-history',
    name: 'Undo, redo & history',
    description: 'Change tracking and audit history',
    icon: 'trend',
  },
  {
    id: 'pivot-analytics',
    name: 'Pivot analytics',
    description: 'Totals, subtotals, drill-down and aggregation',
    icon: 'chartColumn',
  },
  {
    id: 'gantt-scheduler',
    name: 'Gantt Charts',
    description: 'Timelines, tasks, dependencies and events',
    icon: 'calendarDays',
  },
]
...
.spinner {
  animation: tailored-spin 1s linear infinite;
  filter: brightness(0);
}
.success-state {
  display: grid;
